...
import { showConfirm } from './alertSystem'
...
function App() {
  // 1. Hooks de Lógica
  const { session, mostrarLogin, setMostrarLogin, handleLogout } = useAuth()
  const {
  ...
  const {
    canchas, reservas, misReservas,
    cargarDatos, cargarMisReservas, cancelarReserva, adminCambiarEstadoPago
  } = useReservasData(diaActual, session)

  const restoreRef = useRef(null)
  const lastRestoreTimeRef = useRef(0)
  const esAdmin = session?.user?.email === ADMIN_EMAIL

  // 2. Lógica para "Pendientes" (Restore al volver del login)
  useEffect(() => {
    if (session) {
      ...
      if (pendingReserva && pendingDiaIndex) {
        ...
        // Volver a la home si estaba en login
    ...
    cargarMisReservas()
  }, [session, cargarMisReservas, diaIndex, setDiaIndex, setTabActiva, cargarDatos])

  // 3. Restaurar selección específica si existe (continuación del efecto anterior)
  useEffect(() => {
    cargarDatos()
    if (restoreRef.current) {
      const datos = restoreRef.current
      setSeleccion(datos)
      // Forzar inicio de reserva (ya logueado)
  ...
  }, [diaIndex, cargarDatos, setSeleccion, setMostrarConfirmacion, setHoraFiltroMovil])

  // 4. Funciones de Interfaz (Bridge)
  const requerirLogin = async () => {
    ...
    if (confirmar) setMostrarLogin(true)
  }

  const iniciarReserva = (datosReserva = null) => {
  ...
  const handleIrASeccionProtegida = (sec) => {
  ...
  const esHoraPasada = (horaStr) => {
    ...
    return parseInt(horaStr.split(':')[0]) <= ahora.getHours()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 font-sans text-gray-800 dark:text-gray-100 pb-0 pt-16 transition-colors duration-300 flex flex-col">
      ...
      <Navbar
        irASeccion={handleIrASeccionProtegida}
        irAMapa={irAMapa}
        tabActiva={tabActiva}
        esAdmin={esAdmin}
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 flex-grow w-full pb-52">
        ...
        {tabActiva === 'admin' && esAdmin && (
          <Suspense fallback={<div className="flex flex-col items-center justify-center py-20"><div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mb-4"></div><p className="text-gray-500 text-sm font-medium">Cargando panel...</p></div>}>
            <AdminPanel canchas={canchas} reservas={reservas} diaActual={diaActual} userId={session.user.id} onPrevDay={() => setDiaIndex(Math.max(0, diaIndex - 1))} onNextDay={() => setDiaIndex(Math.min(DIAS_DISPONIBLES.length - 1, diaIndex + 1))} onCancel={cancelarReserva} onChangePaymentStatus={adminCambiarEstadoPago} onReloadApp={cargarDatos} />
          </Suspense>
        )}
            ...
            <div className="animate-fade-in">
              ...
              <div className="md:hidden">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-4 no-scrollbar mb-4">
                  {DIAS_DISPONIBLES.map((dia, index) => {
                    ...
                    return (
                      <button key={index} onClick={() => { setDiaIndex(index); setHoraFiltroMovil(null); setMobileDaySelected(true); }} className={`min-w-[70px] h-[60px] rounded-xl flex flex-col items-center justify-center border transition-all ${esSeleccionado ? 'bg-white dark:bg-slate-800 border-blue-500 text-blue-600 dark:text-blue-400 shadow-md' : 'bg-transparent border-transparent text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800'}`}>
                        <span className="text-[10px] font-bold tracking-widest">{dia.movilDia}</span><span className="text-lg font-bold leading-none">{dia.movilNum}</span><span className="text-[10px]">{dia.movilMes}</span>
                      </button>
                    )
                  })}
                </div>
                {!mobileDaySelected && <div className="text-center py-6 text-gray-400 dark:text-gray-500 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl mb-6 bg-gray-50/50 dark:bg-slate-800/30">👆 Selecciona una fecha para ver los horarios</div>}
              </div>

              {/* PASO 2: Selector de Horario (Móvil) */}
              <div id="paso-2-horarios" className={`md:hidden space-y-4 scroll-mt-24 transition-opacity duration-500 ${mobileDaySelected ? 'opacity-100' : 'opacity-0 hidden'}`}>
                ...
                <div className="grid grid-cols-4 gap-3 mb-4">
                  {HORARIOS.map(hora => (
                    <button key={hora} disabled={esHorarioLleno(hora) || esHoraPasada(hora)} onClick={() => setHoraFiltroMovil(hora)} className={`py-3 rounded-lg text-sm font-bold border shadow-sm transition-colors ${(esHorarioLleno(hora) || esHoraPasada(hora)) ? 'bg-gray-100 dark:bg-slate-800 text-gray-300 dark:text-gray-600 cursor-not-allowed opacity-50' : horaFiltroMovil === hora ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-slate-700'}`}>
                      {hora}
                    </button>
                  ))}
                </div>
                {!horaFiltroMovil && <div className="text-center py-6 text-gray-400 dark:text-gray-500 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl mb-6 bg-gray-50/50 dark:bg-slate-800/30">👆 Selecciona un horario para ver las canchas</div>}
              </div>

              {/* PASO 3: Selector de Cancha (Móvil) */}
              ...
              <div className="hidden md:block">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden mb-12 transition-colors">
                  <div className="flex border-b border-gray-200 dark:border-slate-700 p-2 items-center justify-between">
                    <div className="flex items-center gap-4 px-4"><img src={CLUB_CONFIG.emoji} alt="Estadio" className="w-9 h-9 object-cover" /><span className="font-bold text-lg dark:text-white">Calendario</span></div>
                    <div className="flex items-center gap-4">
                      ...
                      <div onClick={abrirCalendario} className="relative cursor-pointer border-b-2 border-blue-500 pb-1 px-2 font-bold text-lg dark:text-white">{diaActual.etiquetaDesktop}<input ref={dateRef} type="date" value={diaActual.fecha} min={minDate} max={maxDate} onChange={handleCalendarChange} className="absolute inset-0 opacity-0 pointer-events-none" /></div>
                  ...
                  <div className="overflow-x-auto">
                    <div className="flex min-w-[800px] border-b border-gray-100 dark:border-slate-700">
                      ...
                      <div className="flex-1 flex">{HORARIOS.map(h => <div key={h} className="flex-1 text-center py-4 text-sm font-medium text-gray-500 dark:text-gray-400 border-l border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">{h}</div>)}</div>
                    </div>
                    <div className="divide-y divide-gray-100 dark:divide-slate-700 min-w-[800px]">
                      {canchas.map(c => (
                        <div key={c.id} className="flex h-28 group hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                          <div className="w-[200px] flex-shrink-0 p-4 border-r border-gray-100 dark:border-slate-700 flex flex-col justify-center"><div className="font-bold text-sm text-gray-800 dark:text-white mb-1">{c.nombre}</div><div className="text-xs text-gray-400">Sintético | Cubierta</div></div>
                          <div className="flex-1 flex">{HORARIOS.map(h => (
                            <div key={h} className="flex-1 border-l border-gray-100 dark:border-slate-700 p-2 relative">
                              {(estaOcupada(c.id, h) || esHoraPasada(h)) ? (<div className="w-full h-full bg-slate-100 dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 cursor-not-allowed relative group"><span className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 text-[10px] bg-black text-white px-2 py-1 rounded whitespace-nowrap mb-1 z-10">{esHoraPasada(h) ? 'Hora pasada' : 'Ocupado'}</span></div>) : (<button onClick={() => { setSeleccion({ canchaId: c.id, hora: h, precio: c.precio_hora, nombre: c.nombre }); }} className={`w-full h-full rounded-md transition-all ${seleccion?.canchaId === c.id && seleccion?.hora === h ? 'bg-blue-500 shadow-md scale-95' : 'hover:bg-blue-200 dark:hover:bg-blue-900/50'}`} />)}
                            </div>
                          ))}
                          </div>
                        </div>
                ...
                <div id="seccion-mapa" className="scroll-mt-20">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 mb-4 transition-colors flex flex-col">
                      <div><h3 className="font-bold mb-4 dark:text-white text-lg">Ubicación</h3><p className="text-gray-600 dark:text-gray-300 mb-6">{CLUB_CONFIG.direccion}</p></div>
                      <a href="https://www.google.com/maps/dir/?api=1&destination=-32.852333,-68.863624" target="_blank" rel="noopener noreferrer" className="w-full bg-blue-100 hover:bg-blue-200 text-blue-700 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-300 py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors mt-auto">Ver en Google Maps</a>
                    </div>
                  </div>
                  <div className="w-full bg-white dark:bg-slate-800 h-[400px] rounded-xl shadow overflow-hidden relative border border-gray-200 dark:border-slate-700">
                    <Map initialViewState={{ longitude: -68.863624, latitude: -32.852333, zoom: 15 }} className="w-full h-full" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className={`${tabActiva === 'mis_turnos' ? 'block' : 'hidden'} animate-fade-in`}>
          ...
          <div className="max-w-3xl"><MyBookings misReservas={misReservas} onCancel={cancelarReserva} /></div>
        </div>

        <div className={`${tabActiva === 'info' ? 'block' : 'hidden'} pt-4 md:hidden`}><InfoSection /></div>
